...
public class AMInteractiveSys  extends AMGeneral{
   ...
   double [] queueLengthUsr;
    int lastTime=0;
   public AMInteractiveSys(InteractiveSystem is)
   {
       this.IS=is;
...
//                  ES.numberOfActiveServ=ES.applicationList.get(0).numberofRunningNode()-1;
    ...
    public void monitor() {
        ...
        for (int i = 0; i < IS.UserList.size(); i++) {
            ...
            percentCompPwr[i]= IS.UserList.get(i).AM.percnt/((Main.localTime-lastTime)*3*IS.UserList.get(i).ComputeNodeList.size());//(Main.epochSys*/*3*ES.applicationList.get(i).ComputeNodeList.size());
            IS.UserList.get(i).AM.percnt=0;
            accuSLA[i]=IS.UserList.get(i).AM.accumulativeSLA/(Main.localTime-lastTime);//Main.epochSys;
...
//        super.utility=sigmoid(globalUtil-100);        
    ...
    void iterativeAlg()
    {
        for(int i=0;i<IS.UserList.size();i++)
        {
            IS.UserList.get(i).AM.StrategyWsitch=Main.strategyEnum.Green; //Green Strategy
            double wkIntensApp;
            ...
            if(percentCompPwr[i]>0.5 && accuSLA[i]>0)
            {
                   ...
                   IS.UserList.get(i).AM.StrategyWsitch=Main.strategyEnum.SLA;//SLA strategy
            }
