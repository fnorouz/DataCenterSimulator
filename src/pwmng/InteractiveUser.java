...
public class InteractiveUser {
    ...
    ArrayList <InteractiveJob> queueWL;
    ...
    public InteractiveUser (GeneralSystem parent)
    {
       ...
       // placement=new jobPlacement(ComputeNodeList);
        AM = (AMInterUser) Main.amTopology.getAM(moType, new Object[]{parent,this});
        this.parent=parent;
    }
  ...
  /*
    ...
    int readWebJob() {
        ...
        if(queueWL.size()>0)
        {
            if (queueWL.get(0).arrivalTimeOfJob==Main.localTime | queueWL.get(0).arrivalTimeOfJob < Main.localTime)
            {
                return 1;
            }
            else return 0;
       ...
       double numberOfWaitingJobs()
    {
        ...
        {
            if(queueWL.get(i).arrivalTimeOfJob<=Main.localTime)
