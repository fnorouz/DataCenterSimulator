...
public class AMApplication3 extends AMGeneral{
    ...
    //int cpAccumu=0;
    Main.strategyEnum StrategyWsitch= Main.strategyEnum.SLA;
    public AMApplication3(EnterpriseSystem Sys, EnterpriseApp app) {
        //dc=dtcenter;
        this.sys=Sys;
        this.app=app;
    }
    ...
    public void monitor(){
        ...
        for(int i=0;i< sys.applicationList.size();i++)
        {
            ...
            {
                if (app.numberOfWaitingJobs()<sys.applicationList.get(i).numberOfWaitingJobs())
                {//this app can generously give one resource to the needed app e.i. index i
    ...
    public void SLAcal(){
        ...
        SLAViolationGen+=accumulativeSLA;
    }
    @Override
    public void analysis(Object violation)
    {
     if(StrategyWsitch==Main.strategyEnum.Green)
    ...
    {
        if(Main.localTime%Main.epochApp!=0)// || Main.localTime<0)
        {
             ...
             for(int j=0;j<app.ComputeNodeList.size();j++)
             {
              ...
              int tedad=app.numberofIdleNode()/2;
               for(int j=0;j<app.ComputeNodeList.size()&& tedad>0 ;j++)
               {
                   if(app.ComputeNodeList.get(j).ready==-1)
                      {
                          System.out.println("App GR: " +app.id +"\tactive a Server!\t\t @"+ Main.localTime + "\tNumber of runinng:  "+app.numberofRunningNode()); 
                          app.ComputeNodeList.get(j).ready=1;
                          app.ComputeNodeList.get(j).Mips=1.4;
                          tedad--;
                           Main.mesg++;
...
//        }
        ...
        System.out.println("app:\t"+app.id+" ----------> :\t\t "+targetApp+"\t\t@:"+Main.localTime +"\tRunning target node= "+sys.applicationList.get(targetApp).numberofRunningNode()+"\tRunning this node= "+app.numberofRunningNode()+"\tstrtgy= "+StrategyWsitch);
        StrategyWsitch=Main.strategyEnum.SLA;
        return true;
    }
}
