...
public class InteractiveSystem extends GeneralSystem {
     ArrayList <InteractiveUser> UserList,waitingQueueWL;
     File logFile;
    public InteractiveSystem (String config, String moType)
    {
        this.moType = moType;
        ...
        rc.initialResourceAloc_sys(this);
        SLAviolation=0;
...
//            AM.execution();
    ...
    int readWL() {
        ...
        if(waitingQueueWL.size()>0)
          {
            if (waitingQueueWL.get(0).arrivalTime==Main.localTime | waitingQueueWL.get(0).arrivalTime< Main.localTime)
                return 1;
            else return 0;
          }
    ...
    void violationCheckandSet() throws IOException
    {
        ...
        if(SLAviolation>0)
        {   Main.SLALogI.write(name+ "\t"+Main.localTime +"\t"+ SLAviolation +"\n");
