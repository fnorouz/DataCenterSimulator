...
public class AMInterUser extends AMGeneral {
    ...
    //int cpAccumu=0;
    Main.strategyEnum StrategyWsitch=Main.strategyEnum.Green; //Green
    public AMInterUser(InteractiveSystem sys,InteractiveUser usr) {
        this.sys=sys;
        this.User=usr;
    }  
    @Override
             ...
             for(int j=0;j<User.ComputeNodeList.size();j++)
             {
              ...
              int tedad=User.numberofIdleNode()/2;
               for(int j=0;j<User.ComputeNodeList.size()&& tedad>0 ;j++)
               {
                   if(User.ComputeNodeList.get(j).ready==-1)
                      {
                          System.out.println("USer GR: " +User.id +"\tactive a Server!\t\t @"+ Main.localTime + "\tNumber of runinng:  "+User.numberofRunningNode()); 
                          User.ComputeNodeList.get(j).ready=1;
                          User.ComputeNodeList.get(j).Mips=1.4;
                          tedad--;
                      }
...
//        }
        ...
        System.out.println("User :\t"+User.id+" ----------> :\t\t "+targetUsr+"\t\t@:"+Main.localTime +"\tRunning target node= "+sys.UserList.get(targetUsr).numberofRunningNode()+"\tRunning this node= "+User.numberofRunningNode()+"\tstrtgy= "+StrategyWsitch);
